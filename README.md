...
A Docker-first Laravel application for temporary PDF/DOCX storage. Files are uploaded asynchronously, stored privately, listed and downloaded through the application, and deleted manually or automatically after 24 hours. Every deletion creates a durable RabbitMQ email-notification event through a transactional outbox.
...
```text
topic exchange: file.events (durable)
...
The SMTP consumer is intentionally out of scope. Delivery is **at least once**. A consumer must deduplicate using `event_id`.
